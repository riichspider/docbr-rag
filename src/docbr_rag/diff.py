...
class DocumentComparator:
    ...
    def compare_documents(
        ...
        try:
            ...
            self.logger.info(
                f"Comparação concluída: similaridade {similaridade:.2f}")
            return result
    ...
    def compare_structured_data(
        ...
        try:
            ...
            similaridade = campos_iguais / \
                len(keys_importantes) if keys_importantes else 1.0
...
class DocumentVersionManager:
    ...
    def compare_versions(
        ...
        version1_info = next(
            (v for v in versions if v.versao == versao1), None)
        version2_info = next(
            (v for v in versions if v.versao == versao2), None)
